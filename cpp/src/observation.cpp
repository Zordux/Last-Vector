...
constexpr float kRayRange = 600.0f;
...
float boundary_hit_t(Vec2 origin, Vec2 dir) {
    ...
    const float t = ray_aabb_t(origin, dir, arena_bounds);
    if (t < 0.0f) {
        return kRayRange;
    }
    return std::clamp(t, 0.0f, kRayRange);
}
...
std::vector<float> build_observation(const GameState& state) {
    ...
    if (choosing) {
        constexpr float denom = static_cast<float>(UpgradeId::Count) - 1.0f;
        obs.push_back(static_cast<float>(state.upgrade_offer[0]) / denom);
        obs.push_back(static_cast<float>(state.upgrade_offer[1]) / denom);
        obs.push_back(static_cast<float>(state.upgrade_offer[2]) / denom);
    } else {
